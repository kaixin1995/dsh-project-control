...
| **执行中心** | 页面直接创建变更任务：LLM 生成计划 → 后台子代理逐步执行 → 运行历史与成本 |
| **Review 问题** | 全项目问题看板：按级别统计筛选、按状态过滤；**复检闭环**（见下）；附验收记录 |
| **笔记与记忆** | 项目笔记（标签 / 置顶 / 搜索 / 编辑痕迹）+ 增量 AI 学习总结 + 项目记忆（人工确认、按分支筛选）+ 学习概念 |
...
只有判定 FIXED 才自动置为已解决（卡片保留复检依据）；未修复的保持待处理并附「仍未修复」原因。
...
| 计划与执行 | `create_plan`（版本化）/ `start_run`：每步骤独立子代理执行 + 工作区真值校验 + 重试/成本预算，**AI 不能自己宣布成功** |
| 已确定约束 | 人工确认的需求/约束/禁改路径；AI 写禁改路径在 `tools/pre-execute` 被拒（按项目根限定，相对/绝对路径均可） |
| 项目记忆 | `record_memory`（analysis 级）/ 人工确认升级 confirmed / `recall_project` 有据回答；执行时自动注入相关上下文，按项目+分支组织 |
