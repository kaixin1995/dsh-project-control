...
| 页签 | 内容 |
|---|---|
| **提交核查** | 核心页：顶部多选提交（含未提交改动）→ 改了什么 / 实现逻辑 / 风险点解读 → 影响范围图（哪些函数被改、调用方受何影响，附 LLM 逐函数说明）→ 最优性核查（结构化中文问题清单）→ 文件级高亮 diff 对比（默认折叠）→ 多选 ≥2 时**工作轮次叙事**（整轮工作分几步、每步对应哪些提交）→ 一键记笔记/沉淀记忆 |
| **项目总览** | 项目档案 / 引导检查点 / 证据流 / 已确定约束（AI 禁改拦截） |
| **执行中心** | 中控驾驶舱：计划确认页（每步角色/模型/失败策略可调）→ 后台子代理逐步执行（完成协议催促 + 静默 3 分钟停滞检测）→ Run 详情时间线（步骤/模型/成本/注入上下文/决策日志）→ 暂停问人/恢复/断点续跑 → **例行任务**（定时评审/总结/执行，60s 扫描到点自动触发）；顶栏常驻 ▶运行中/✗需处理 徽标 |
| **Review 问题** | 全项目问题看板：按级别统计筛选、按状态过滤；**复检闭环**（见下）；误报可人工「判定误报」关闭；附验收记录 |
| **笔记与记忆** | 项目笔记（标签 / 置顶 / 搜索 / 编辑痕迹 / 复制 Markdown）+ 增量 AI 学习总结（带消化度提示：上次总结后有 N 个新提交）+ **项目记忆**（血缘 / 有效性状态 / 待确认队列 / 分支作用域 / 拉取同步 / Run 自动提炼）+ 学习概念 |
...
只有判定 FIXED 才自动置为已解决（卡片保留复检依据）；未修复的保持待处理并附「仍未修复」原因。已解决问题默认保留 7 天后自动清理（`resolvedIssueRetentionDays` 可配，`0` 为永久保留）。
...
| 已确定约束 | 人工确认的需求/约束/禁改路径；**事前告知**（执行步骤开工前注入禁改清单）+ 事后拦截（`tools/pre-execute` 拒绝写禁改路径，按项目根限定，相对/绝对路径均可）双保险 |
...
dsh plugin --profile web add github:kaixin1995/dsh-project-control#v0.3.0     # 锁定版本（分享时推荐）
