...
function WorkspaceFrame(props) {
  ...
  const detailQueueRef = (0, import_react2.useRef)(Promise.resolve());
  const loadDetail = (target, force) => {
    setDetailStatus((previous) => previous[target] !== void 0 ? previous : { ...previous, [target]: "queued" });
    detailQueueRef.current = detailQueueRef.current.then(() => loadDetailOnce(target, force)).catch(() => {
    });
  };
